...
if __name__ == '__main__':
    ...
    for split in ['train', 'eval']:
        ...
        words.update([token for instance in dataset for token in instance["text"].split()])
        
    intent2idx = {tag: i for i, tag in enumerate(intents)}
    intent_tag_path = os.path.join(opt.output_dir, "intent2idx.json")

    with open(intent_tag_path, 'w') as file:
        json.dump(intent2idx, file)
